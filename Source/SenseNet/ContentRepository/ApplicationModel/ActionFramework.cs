...
namespace SenseNet.ApplicationModel
{
    public class ActionFramework
    {
        ...
        public static IEnumerable<ActionBase> GetActions(Content context, string scenario)
        {
            var backUrl = HttpContext.Current != null ? HttpUtility.UrlEncode(HttpContext.Current.Request.RawUrl) : null;

            return GetActions(context, scenario, backUrl);
        }

        public static IEnumerable<ActionBase> GetActions(Content context, string scenario, string backUri)
        {
        ...
        {
            var permList = new List<PermissionType>();
            var perms = app == null ? null : app.RequiredPermissions;
            
            if (string.IsNullOrEmpty(perms))
                return permList;

            var permArray = perms.Split(new[] {',', ';'}, StringSplitOptions.RemoveEmptyEntries);

            foreach (var permIdString in permArray)
            {
                var permId = 0;

                if (!int.TryParse(permIdString, out permId))
                    continue;
                
                var perm = ActiveSchema.PermissionTypes.GetItemById(permId);
        ...
        public static List<PermissionType> GetRequiredPermissions(string permissionNames)
        {
            var permList = new List<PermissionType>();
            ...
            var permArray = permissionNames.Split(new[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries);

            permList.AddRange(permArray.Select(permName => ActiveSchema.PermissionTypes[permName]).Where(perm => perm != null));
