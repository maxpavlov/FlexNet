...
namespace SenseNet.ApplicationModel
{
    ...
    public class Application : GenericContent
    {
        ...
        private static readonly string[] EXCLUDED_OVERRIDE_PROPERTIES = { "Name", "Path", "Id", "NodeType", "ContentListId", "ContentListType", "Parent", "IsModified", "IsDeleted", "Version","VersionId", 
                                                                          "NodeCreationDate", "NodeModificationDate", "NodeCreatedBy", "NodeModifiedBy", "CreationDate", "ModificationDate", "CreatedById", 
                                                                          "ModifiedById", "Locked", "Lock","LockedById","LockedBy", "LockType", "LockTimeout", "LockDate", "LockToken", "LastLockUpdate", "Security" };
        ...
        [RepositoryProperty("Icon", RepositoryDataType.String)]
        public virtual string Icon
        {
            get { return this.GetOverriddenProperty<string>("Icon"); }
            set { this["Icon"] = value; }
        }
        ...
        public virtual string RequiredPermissions
        {
            get { return this.GetOverriddenProperty<string>("RequiredPermissions"); }
                    ...
                    lock (_customParamObject)
                    {
                        if (_customParams == null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(CustomUrlParameters))
                            {
                                ...
                                foreach (var pair in cups.Select(cup => cup.Split(new[] {"="}, StringSplitOptions.RemoveEmptyEntries))
                                            .Where(pair => pair.Length == 2 && !string.IsNullOrEmpty(pair[0])))
                                {
                                    if (localDict.ContainsKey(pair[0]))
                                        localDict[pair[0]] = pair[1];
                                    else
                                        localDict.Add(pair[0], pair[1]);
                                }
                            }
        ...
        public override void SetProperty(string name, object value)
        {
            switch (name)
            {
                ...
                case "StyleHint":
                    ...
                    break;
                case "RequiredPermissions":
                    this.RequiredPermissions = (string)value;
                    break;
                case "DeepPermissionCheck":
        ...
        {
            return  this["ActionTypeName"] as string;
        }
