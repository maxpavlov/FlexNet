...
namespace  SenseNet.ContentRepository.Schema
{
	...
	{
        [Obsolete("Use typeof(ContentType).Name instead.", true)]
        public static readonly string NodeTypeName = typeof(ContentType).Name;
		...
		public BinaryData Binary
		{
			...
			set
			{
                //XPathDocument doc = new XPathDocument(new StringReader(StringFromStream(value.GetStream())));
                //CheckValidation(doc, value.FileName);
        ...
        internal List<ContentType> ChildTypes { get; private set; }
		...
		private void Build(IXPathNavigable definitionXml)
		{
			...
			SetFieldSlots();
            //AssertValidAnalyzersAndParsers();
		}
			...
			foreach (XPathNavigator subElement in contentTypeElement.SelectChildren(XPathNodeType.Element))
			{
				switch (subElement.LocalName)
				{
                    ...
                    case "AllowIncrementalNaming":
                        ...
                        break;
                    case "AppInfo":
                        this._appInfo = subElement.InnerXml;
                        break;
        ...
        {
            SetFieldSlots(TypeHandler.GetType(this.HandlerName));
        }
