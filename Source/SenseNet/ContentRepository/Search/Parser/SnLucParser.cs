...
namespace SenseNet.Search.Parser
{
    ...
    internal class SnLucParser
    {
        ...
        private class FieldInfo
        {
            ...
            public string Name { get; set; }
            //public IndexableDataType DataType { get; set; }
        ...
        private IDictionary<string, PerFieldIndexingInfo> _indexingInfo;
        private FieldIndexHandler GetFieldParser(string fieldName)
        {
            PerFieldIndexingInfo info;
            if (_indexingInfo.TryGetValue(fieldName, out info))
                return info.IndexFieldHandler;
            return null;
        }
        ...
        public SnLucParser()
        {
            _indexingInfo = SenseNet.ContentRepository.Schema.ContentTypeManager.GetPerFieldIndexingInfo();
        ...
        private Query ParseBinaryTermExp()
        {
            ...
            switch (fieldInfo.OperatorToken)
            {
                case SnLucLexer.Token.LT:
                    //return new TermRangeQuery(fieldInfo.Name, null, value.StringValue, true, false);
                    return CreateRangeQuery(fieldInfo.Name, null, value, true, false);
                case SnLucLexer.Token.LTE:
                    //return new TermRangeQuery(fieldInfo.Name, null, value.StringValue, true, true);
                    return CreateRangeQuery(fieldInfo.Name, null, value, true, true);
                case SnLucLexer.Token.GT:
                    //return new TermRangeQuery(fieldInfo.Name, value.StringValue, null, false, true);
                    return CreateRangeQuery(fieldInfo.Name, value, null, false, true);
                case SnLucLexer.Token.GTE:
                    //return new TermRangeQuery(fieldInfo.Name, value.StringValue, null, true, true);
                    return CreateRangeQuery(fieldInfo.Name, value, null, true, true);
            }

            throw new NotImplementedException("Unexpected OperatorToken: " + fieldInfo.OperatorToken);
        }
        ...
        private FieldInfo ParseFieldHead()
        {
            ...
            {
                fieldInfo = new FieldInfo { Name = _lexer.StringValue };
        ...
        private QueryFieldValue ParseExactValue(bool throwEx)
        {
            ...
            var fieldName = _currentField.Peek().Name;
            ...
            {
                var fieldHandler = GetFieldParser(fieldName);
        ...
        {
            return new ParserException(String.Concat(msg, " (query: \"", _lexer.Source, "\")"), _lexer.CreateLastLineInfo());
        }
    }
}
