...
Debug.WriteLine(String.Format("@> {0} =========== BackupIndex END. id: {1}", AppDomain.CurrentDomain.FriendlyName, SenseNet.ContentRepository.Storage.Data.RepositoryConfiguration.IndexBackupCreatorId));
    ...
    [Serializable]
    public abstract class MachineAndInstanceDependentMessage : ClusterMessage
    {
        private string _machine;
        public string Machine { get { return _machine; } }
        private string _instanceId;
        public string InstanceId { get { return _instanceId; } }
        public MachineAndInstanceDependentMessage(string machine, string instanceId)
        {
            _machine = machine;
    ...
    [Serializable]
    public sealed class RestoreIndexRequestMessage : DistributedAction
    {
        public override void DoAction(bool onRemote, bool isFromMe)
        {
            if (onRemote && isFromMe)
...
Debug.WriteLine(String.Format("@> {0} =========== Executing Restore action is finished.", AppDomain.CurrentDomain.FriendlyName));
    ...
    [Serializable]
    public sealed class IndexRestoringStartedMessage : MachineAndInstanceDependentMessage
    {
        public IndexRestoringStartedMessage(string machine, string instanceId) : base(machine, instanceId) { }
    }
    [Serializable]
    public sealed class IndexRestoringFinishedMessage : MachineAndInstanceDependentMessage
    {
        public IndexRestoringFinishedMessage(string machine, string instanceId) : base(machine, instanceId) { }
    }
    [Serializable]
    public sealed class IndexRestoringErrorMessage : MachineAndInstanceDependentMessage
    {
        ...
        public IndexRestoringErrorMessage(string machine, string instanceId, Exception ex)
            : base(machine, instanceId)
        {
            if (ex.GetType().GetCustomAttributes(typeof(SerializableAttribute), false).FirstOrDefault() != null)
                _exception = ex;
            else
                ex = new ApplicationException(String.Concat(
                    "An exception occured during restoring the index. Original exception is not serializable. Original exception: ",
...
<RestoreInfo>
  <BackupId>{0}</BackupId>
        ...
        internal static void CopyIndexToBackupDirectory() // caller: BackupActivity.Execute() (synchron)
        {
            ...
            //Progress.SetAndNotify(IndexBackupProgressType.CopyIndexToBackup, "Copying index to backup directory", 0, 1);
        ...
        internal static void OptimizeCompressAndStore() // caller: BackupIndexImmediatelly (synchron), BackupActivity.Execute() (asynchron)
        {
            ...
            Logger.WriteInformation("Index directory is successfully backed up.");
        }
