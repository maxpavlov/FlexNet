    ...
    public class Voting : Survey
    {
        ...
        public bool IsVotingAvailable
        {
            get
            {
                return Convert.ToBoolean(this["EnableMoreFilling"]) || 
                       ContentQuery.Query(string.Format("+Type:votingitem +CreatedById:{0} +InTree:\"{1}\" .AUTOFILTERS:OFF .COUNTONLY", User.Current.Id, this.Path)).Count <= 0;
            }
        }
        ...
        public Dictionary<string,string> Result
        {
            get
            {
                ...
                {
                    var count = ContentQuery.Query(string.Format("+Type:votingitem +InTree:\"{0}\" +{1}:{2} .AUTOFILTERS:OFF .COUNTONLY", this.Path, questionName, question.Key)).Count;
