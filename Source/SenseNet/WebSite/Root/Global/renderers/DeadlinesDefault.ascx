      ...
      <ItemTemplate>
        <asp:PlaceHolder id="plcCalendarItem" runat="server">
            <div class="sn-deadline-daysleft">
                ...
                <span><%# Eval("Task_DueText") %></span>
            </div>
            <p class="sn-deadline-details"><%# Eval("GenericContent_DisplayName") %></p>
            <div><a class="sn-deadline-detailslink" href='<%# SenseNet.Portal.Virtualization.PortalContext.Current.GetContentUrl(Container.DataItem) %>'>Details</a></div>
        </asp:PlaceHolder>
      </ItemTemplate>
      <EmptyDataTemplate>        
      </EmptyDataTemplate>	  
