...
namespace SenseNet.ContentRepository.Storage.Schema
{
	public abstract class SchemaRoot : ISchemaRoot
	{
		...
		private TypeCollection<PermissionType> _permissionTypes;
		...
		public TypeCollection<PermissionType> PermissionTypes
		{
		...
		public void Load()
		{
            using (var traceOperation = Logger.TraceOperation("Load storage schema."))
            {
                Clear();
		...
		private void Load(DataSet dataSet)
		{
			...
			BuildPermissions(dataSet.Tables["Permissions"]);
		...
		private static DataSet CreateDataSet(Dictionary<string, int> dataTypes, Dictionary<PropertySetType, int> propertySetTypes)
		{
			...
			//-----------------------------------------------
			table = dataSet.Tables.Add("Permissions");
		...
		private static void BuildPermissionRow(XmlNode node, DataTable table)
		{
			//table = dataSet.Tables.Add("Permissions");
			//table.Columns.Add("PermissionID", typeof(int));
			//table.Columns.Add("Name", typeof(string));

			//<Permission itemID="4" name="Permission4" />
		...
		{
			//-- letezik ilyen nevu slot: exception
			//-- letezik mas sloton ilyen mapping: exception
		...
		public PermissionType CreatePermissionType(string name)
		{
			return CreatePermissionType(0, name);
		}
		private PermissionType CreatePermissionType(int id, string name)
		{
			PermissionType perm = new PermissionType(id, name, this);
			this.PermissionTypes.Add(perm);
			return perm;
		}
