...
namespace SenseNet.ContentRepository.Tests.Schema
{
    ...
    public class FieldSettingTest : TestBase
    {
        ...
        public void FieldValidation_ShortText()
        {
            ...
            Assert.IsTrue(validationResult.Category == ShortTextFieldSetting.MinLengthName, "#3");
            Assert.IsTrue((int)content.Fields["ShortTextTest"].ValidationResult.GetParameter(ShortTextFieldSetting.MinLengthName) == 3, "#4");

            content["ShortTextTest"] = "abcdefgh01";
            isValid = content.IsValid;
            Assert.IsFalse(isValid, "#5");
            Assert.IsTrue(content.Fields["ShortTextTest"].ValidationResult.Category == ShortTextFieldSetting.MaxLengthName, "#6");
            Assert.IsTrue((int)content.Fields["ShortTextTest"].ValidationResult.GetParameter(ShortTextFieldSetting.MaxLengthName) == 8, "#7");

            content["ShortTextTest"] = "!@#$%^";
            isValid = content.IsValid;
            Assert.IsFalse(isValid, "#8");
            Assert.IsTrue(content.Fields["ShortTextTest"].ValidationResult.Category == ShortTextFieldSetting.RegexName, "#9");
        ...
        [TestMethod]
        public void FieldValidation_PasswordAndControl()
        {
            string ctd;
            bool isValid;
            FieldValidationResult validationResult;
            ...
            PrivateType fieldSettingAccessor = new PrivateType(typeof(PasswordFieldSetting));
            var defaultStars = SenseNet.Portal.UI.Controls.Password.DefaultStars;
            //====
            ...
            SNC.Content content = SNC.Content.CreateNew("ValidatedContent", _testRoot, "abc");
            var pwdField = content.Fields["PasswordTest"];
            var pwdControl = new SenseNet.Portal.UI.Controls.Password();
            var pwdControlAcc = new PasswordAccessor(pwdControl);
            pwdControlAcc.ConnectToField(pwdField);
            pwdControlAcc.SetDataInternal();

            //-- stars are not overridden and original is invalid (null)
            pwdControlAcc.InputTextBox.Text = defaultStars;
            pwdControlAcc.InputTextBox2.Text = defaultStars;
            pwdField.SetData(pwdControl.GetData());

            isValid = content.IsValid;
            validationResult = content.Fields["PasswordTest"].ValidationResult;
            Assert.IsFalse(isValid, "#1");
            Assert.IsTrue(validationResult.Category == PasswordFieldSetting.OriginalIsInvalidError, "#2");

            //-- inputs are not eqals
            pwdControlAcc.InputTextBox.Text = "asdf";
            pwdControlAcc.InputTextBox2.Text = "qwer";
            var expectedExceptionWasThrown = false;
            try
            {
                pwdField.SetData(pwdControl.GetData());
            }
            catch (SenseNet.Portal.UI.Controls.FieldControlDataException)
            {
                expectedExceptionWasThrown = true;
            }
            isValid = content.IsValid;
            validationResult = content.Fields["PasswordTest"].ValidationResult;
            Assert.IsTrue(expectedExceptionWasThrown, "#3");
            Assert.IsFalse(isValid, "#4");

            //-- correct #1
            pwdControlAcc.InputTextBox.Text = "zxcv";
            pwdControlAcc.InputTextBox2.Text = "zxcv";
            pwdField.SetData(pwdControl.GetData());

            isValid = content.IsValid;
            validationResult = content.Fields["PasswordTest"].ValidationResult;
            Assert.IsTrue(isValid, "#6");
            Assert.IsTrue(validationResult == FieldValidationResult.Successful, "#7");

            //-- unchanged control does not change the content

            content.Save();
            var contentId = content.Id;
            var hash = ((PasswordField.PasswordData)content.Fields["PasswordTest"].OriginalValue).Hash;

            SNC.Content content2 = SNC.Content.Load(contentId);
            pwdField = content.Fields["PasswordTest"];
            pwdControl = new SenseNet.Portal.UI.Controls.Password();
            pwdControlAcc = new PasswordAccessor(pwdControl);
            pwdControlAcc.ConnectToField(pwdField);
            pwdControlAcc.SetDataInternal();
            //-- user does not change the values
            pwdField.SetData(pwdControl.GetData());
            isValid = content.IsValid;
            validationResult = content.Fields["PasswordTest"].ValidationResult;
            Assert.IsTrue(isValid, "#8");
            Assert.IsTrue(validationResult == FieldValidationResult.Successful, "#9");
            content.Save();
            SNC.Content content3 = SNC.Content.Load(contentId);
            var hash3 = ((PasswordField.PasswordData)content3["PasswordTest"]).Hash;
            Assert.IsTrue(hash == hash3, "#10");

            //-- setting back original value
            content["PasswordTest"] = new PasswordField.PasswordData { Hash = hash };
            isValid = content.IsValid;
            content.Save();
            SNC.Content content4 = SNC.Content.Load(contentId);
            var hash4 = ((PasswordField.PasswordData)content4["PasswordTest"]).Hash;
            Assert.IsTrue(hash == hash4, "#11");
        ...
        public void FieldValidation_Choice()
        {
            ...
            Assert.IsTrue(result == "1;4;SeventyTwo", "#2");
            ...
            Assert.IsTrue(result == "1;4;SeventyTwo", "#5");
            ...
            Assert.IsTrue(result == "SeventyTwo", "#13");
        ...
        public void FieldValidation_Integer()
        {
            ...
            SenseNet.Portal.UI.Controls.WholeNumber intEd = new SenseNet.Portal.UI.Controls.WholeNumber();
        ...
        public void FieldValidation_Number()
        {
            ...
            SenseNet.Portal.UI.Controls.Number numberEd = new SenseNet.Portal.UI.Controls.Number();
...
<Digits>2</Digits>
        ...
        {
            var field1 =@"<Field name=""TestField1"" type=""Reference""><DisplayName>Test name</DisplayName><Configuration /></Field>";
            var field2 =
