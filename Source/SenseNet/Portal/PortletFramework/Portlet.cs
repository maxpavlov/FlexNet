...
namespace SenseNet.Portal.UI.PortletFramework
{
    ...
    public abstract class Portlet : WebPart
    {
        ...
        private void RenderTimerValue(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            sb.Append(String.Format("(from cache:{5}) Execution time of the {1} portlet was {0:F10} seconds. Cacheable:{2}, CanCache:{3}, IsInCache:{4}", _timer.Elapsed.TotalSeconds, this.ID, this.Cacheable.ToString(), this.CanCache.ToString(), this.IsInCache.ToString(), this.fromCache.ToString()));
            sb.Append(@"</div>");
