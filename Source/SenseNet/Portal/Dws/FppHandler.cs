...
namespace SenseNet.Portal.Dws
{
    public class FppHandler : IHttpHandler
    {
        ...
        private const string ENTRYPOINTSTR = @"<!-- FrontPage Configuration Information FPVersion=""14.00.0.000""
...
{0}
        ...
        private void HandleUrlToWebUrl(HttpContext context)
        {
            ...
            if (path != "/")
            {
                ...
                if (node != null)
                {
                    var doclib = Node.GetAncestorOfNodeType(node, "DocumentLibrary");
                    if (doclib != null)
                    {
        ...
        private void HandleAuthorOpenService(HttpContext context)
        {
            ...
            var servertz = DateTime.UtcNow.ToString("zz") + TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now).ToString("mm"); // "+0100";
