...
namespace SenseNet.Portal.ApplicationModel
{
    ...
    public class ImgResizeApplication : Application, IHttpHandler
    {
        ...
        /*
        [RepositoryProperty("Title")]
        public string Title
        {
            get { return base.GetProperty<string>("Title"); }
            set { base.SetProperty("Title", value); }
        }
         */
        ...
        public override void SetProperty(string name, object value)
        {
            ...
            {
                case "imagetype":
                    this.ImageType = value.ToString();
                    break;
                case "imagefieldname":
                    this.ImageFieldName = value.ToString();
                    break;
                case "width":
                    try
                    {
                        int w = Int32.Parse(value.ToString());
                ...
                case "height":
                    try
                    {
                        int h = Int32.Parse(value.ToString());
                ...
                case "stretch":
                    try
                    {
                        this.Stretch = Boolean.Parse(value.ToString());
                    }
                    catch (Exception)
                    {
                    ...
                    break;
                case "outputformat": this.ResizeOutputFormat = GetImageFormat(value.ToString()); break;
                case "smoothingmode": this.ResizeSmoothingMode = (SmoothingMode)Enum.Parse(ResizeSmoothingMode.GetType(), value.ToString(), true); break;
                case "interpolationmode": this.ResizeInterpolationMode = (InterpolationMode)Enum.Parse(ResizeInterpolationMode.GetType(), value.ToString(), true); break;
                case "pixeloffsetmode": this.ResizePixelOffsetMode = (PixelOffsetMode)Enum.Parse(ResizePixelOffsetMode.GetType(), value.ToString(), true); break;
                case "resizetypemode": this.ResizeType = (ResizeTypeList)Enum.Parse(ResizeType.GetType(), value.ToString(), true); break;
                case "cropvalign": 
                        this.CropVAlign = value.ToString();
                    break;
                case "crophalign": 
                        this.CropHAlign = value.ToString();
                    break;
                default:
        ...
        private string AppCacheFolder
        {
            get
            {
                ...
                var cacheFolder = System.Configuration.ConfigurationManager.AppSettings["ResizedImagesCacheFolder"];
        ...
        private void FlushCachedFile(string imageNodePath, HttpContext context)
        {
            ...
            Stream imageStream = System.IO.File.Open(fileName, FileMode.Open, FileAccess.Read);//Open the file ReadOnly mode
